DROP DATABASE IF EXISTS SpotifyClone;
CREATE DATABASE SpotifyClone;

CREATE TABLE SpotifyClone.plano(
id_plano INT NOT NULL AUTO_INCREMENT,
plano VARCHAR(100) NOT NULL,
preço DOUBLE NOT NULL,
PRIMARY KEY (id_plano)
) engine = InnoDB;

INSERT INTO SpotifyClone.plano( plano, preço)
VALUES
('gratuito', 0.00),
('familia', 7.99),
('universitario', 5.99),
('pessoal', 6.99);



CREATE TABLE IF NOT EXISTS SpotifyClone.usuario (
  id_usuario INT NOT NULL AUTO_INCREMENT,
  idade INT NOT NULL,
  nome VARCHAR(100) NOT NULL,
  plano_id_plano INT NOT NULL,
  data_assinatura VARCHAR(45) NOT NULL,
  PRIMARY KEY (id_usuario),
    FOREIGN KEY (plano_id_plano)
    REFERENCES SpotifyClone.plano (id_plano)
    ) engine = InnoDB;

INSERT INTO SpotifyClone.usuario( nome,plano_id_plano, idade, data_assinatura)
VALUES
('Barbara Liskov', 1, 82, '2019-10-20' ),
('Robert Cecil Martin', 1, 58, '2017-01-06'),
('Ada Lovelace', 2, 37, '2017-12-30'),
('Martin Fowler', 2,  46, '2017-01-17'),
('Sandi Metz', 2,  58, '2018-04-29'),
('Paulo Freire', 3, 19, '2018-02-14'),
('Bell Hooks', 3,  26, '2018-01-05'),
('Christopher Alexander', 4,  85, '2019-06-05'),
('Judith Butler', 4, 45, '2020-05-13'),
('Jorge Amado', 4, 58, '2017-02-17');





CREATE TABLE IF NOT EXISTS SpotifyClone.artista (
  id_artista INT NOT NULL AUTO_INCREMENT,
  nome VARCHAR(100) NOT NULL,
  PRIMARY KEY (id_artista)
 ) engine = InnoDB;

INSERT INTO SpotifyClone.artista(nome)
VALUES
('Beyoncé'),
('Queen'),
('Elis Regina'),
('Baco Exu do Blues'),
('Blind Guardian'),
('Nina Simone');




CREATE TABLE IF NOT EXISTS SpotifyClone.albums (
  id_album INT NOT NULL AUTO_INCREMENT,
  id_artista INT NOT NULL,
  album VARCHAR(100) NOT NULL,
  PRIMARY KEY (id_album),
    FOREIGN KEY (id_artista)
    REFERENCES SpotifyClone.artista (id_artista)
) engine = InnoDB;

INSERT INTO SpotifyClone.albums(album, id_artista)
VALUES
('Renaissance', 1),
('Jazz', 2),
('Hot Space', 2),
('Falso Brilhante', 3),
('Vento de Melo', 3),
('QVVJFA?', 4),
('Somewhere Far Beyond', 5),
('I Put A Spell On You', 6);


CREATE TABLE IF NOT EXISTS SpotifyClone.cancoes (
  id_cancoes INT NOT NULL AUTO_INCREMENT,
  id_album  INT NOT NULL,
  nome VARCHAR(100) NOT NULL,
  duracao INT NOT NULL,
  ano YEAR NOT NULL,
  PRIMARY KEY (id_cancoes),
    FOREIGN KEY (id_album)
    REFERENCES SpotifyClone.albums (id_album)
    ) engine = InnoDB;
    




INSERT INTO SpotifyClone.cancoes(nome, duracao, ano, id_album )
VALUES
("BREAK MY SOUL", 279,2022, 1),
("VIRGO’S GROOVE", 369,2022,1),
("ALIEN SUPERSTAR",116,2022,1),
('Don’t Stop Me Now', 203, 1978,2),
('Under Pressure', 152, 1982,2),
('Como Nossos Pais', 105, 1998,3),
('O Medo de Amar é o Medo de Ser Livre', 207, 2001,3),
('Samba em Paris', 267, 2003,4),
('The Bard’s Song', 244, 2007,5),
('Feeling Good', 100,2012,6);


CREATE TABLE IF NOT EXISTS SpotifyClone.historico_de_reproducoes (
  id_usuario INT  NOT NULL,
  id_cancao INT  NOT NULL,
  data_reproducoes DATETIME NOT NULL,
 CONSTRAINT PRIMARY KEY (id_usuario, id_cancao),
 FOREIGN KEY (id_usuario) REFERENCES SpotifyClone.usuario (id_usuario),
 FOREIGN KEY (id_cancao) REFERENCES SpotifyClone.cancoes (id_cancoes)
  ) engine= InnoDB;

INSERT INTO SpotifyClone.historico_de_reproducoes(id_usuario, id_cancao, data_reproducoes)
VALUES
(1, 8, '2022-02-28 10:45:55'),
(1, 2, '2020-05-02 05:30:35'),
(1, 10, '2020-03-06 11:22:33'),
(2, 10, '2022-08-05 08:05:17'), 
(2, 7, '2020-01-02 07:40:33'),
(3, 10,'2020-11-13 16:55:13'), 
(3, 2,'2020-12-05 18:38:30'),
(4, 8, '2021-08-15 17:10:10'),
(5, 8, '2022-01-09 01:44:33'),
(5, 5, '2020-08-06 15:23:43'),
(6,7, '2017-01-24 00:31:17'),
(6, 1, '2017-10-12 12:35:20'),
(7, 4, '2011-12-15 22:30:49'),
(8, 4, '2012-03-17 14:56:41'),
(9, 9, '2022-02-24 21:14:22'),
(10, 3, '2015-12-13 08:30:22');





CREATE TABLE IF NOT EXISTS SpotifyClone.seguidores(
  fk_usuario INT  NOT NULL,
  fk_artista INT NOT NULL,
 CONSTRAINT PRIMARY KEY (fk_usuario, fk_artista),
  FOREIGN KEY (fk_usuario) REFERENCES SpotifyClone.usuario (id_usuario), 
  FOREIGN KEY (fk_artista) REFERENCES SpotifyClone.artista (id_artista)
) engine = InnoDB;


INSERT INTO SpotifyClone.seguidores( fk_usuario, fk_artista )
VALUES
(1, 1),
(1, 2),
(1, 3),
(2, 1),
(2, 3),
(3, 2),
(4, 4),
(5, 5),
(5, 6),
(6, 6),
(6, 1),
(7, 6),
(9, 3),
(10, 2);




